...
object TypedBigQueryIT {
  ...
  case class Record(timestamp: Instant)

  case class RecordX(bool: Boolean, int: Int, long: Long, float: Float, double: Double,
...
class TypedBigQueryIT extends PipelineSpec with BeforeAndAfterAll {
  ...
  "TypedBigQuery" should "work with Avro input" in {
    val sc = ScioContext(options)
    sc.typedBigQuery[Record](table, useAvro = true) should containInAnyOrder (records)
    sc.close()
  }

  it should "work with TableRow input" in {
    val sc = ScioContext(options)
    sc.typedBigQuery[Record](table, useAvro = false) should containInAnyOrder (records)
    sc.close()
  }
